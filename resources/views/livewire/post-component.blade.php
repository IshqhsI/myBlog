<div class="w-full">
    ...
    <div class="flex">
        ...
        <div class="w-1/2 text-end">
            <input type="text" class="form-control w-full md:w-1/2 rounded p-2 border-blue-600" placeholder="Search"
                wire:keydown="searchPost" wire:model="search">
        </div>
    </div>
    ...
    <div class="overflow-x-auto">
        <table
            class="min-w-full bg-white shadow-md rounded-lg overflow-hidden dark:bg-gray-800 dark:text-gray-100 mt-5">
                    ...
                    @foreach ($posts as $i => $post)
                        <tr class="border-b dark:border-gray-600">
                            ...
                            <td class="py-3 px-6 text-left">
                                {{ \Illuminate\Support\Str::words($post->content, 3, '...') }}</td>
                            <td class="py-3 px-6 text-left"><img src="{{ asset('/storage/posts/' . $post->image) }}"
                                    width="200" alt=""></td>
                            <td class="py-3 px-6 text-left">{{ $post->category->name }}</td>
                            <td class="py-3 px-0 text-left">
                                <select
                                    class="inline-block w-full mt-1 text-sm border-gray-300 dark:border-gray-600 rounded-lg shadow-sm focus:ring-indigo-500 focus:border-indigo-500 dark:bg-gray-800 dark:text-gray-300"
                                    data-post-id="{{ $post->id }}" wire:model="post.status" wire:change="updateStatusPost($event.target.value, {{ $post->id }})">
