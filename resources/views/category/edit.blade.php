    ...
    <div class="py-10">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="mx-auto bg-white p-6 rounded-lg shadow-md dark:bg-gray-800">
                <h5 class="text-lg font-medium mb-6 dark:text-gray-100 text-gray-700">Edit Category</h5>
                <form action="{{ route('categories.update', $category->id) }}" method="post" class="space-y-6">
                    ...
                    <div class="mb-4 ">
                        ...
                        <input type="text" id="name" name="name"
                            ...
                            wire:model="name" value="{{ $category->name }}">
                        @error('name')
                    ...
                    <div class="mb-4">
                        ...
                        <textarea id="description" name="description" rows="4"
                            class="form-control block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md shadow-sm focus:outline-none focus:ring-indigo-500 dark:focus:ring-indigo-600 focus:border-indigo-500 dark:focus:border-indigo-600 sm:text-sm dark:bg-gray-700 dark:text-gray-100 @error('description') border-red-500 dark:border-red-600  @enderror"
                            wire:model="description">{{ $category->description }}</textarea>
                        @error('description')
                            <span class="text-red-500 text-sm dark:text-red-400">{{ $message }}</span>
                        @enderror
                    </div>
